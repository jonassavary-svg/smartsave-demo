(() => {
  ...
  const renderActions = (data, formData, activeUser) => {
    ...
    const monthInfo = activeUser ? getActiveMonthEntry(activeUser, formData) : null;
    const activeMonthKey = monthInfo?.activeKey || getMonthKey(new Date());
    ...
    const startingBalances = normalizeBalances(
      monthInfo?.month?.startingBalances || resolveBalances(formData || {})
    );
    const monthTransactions = getMonthTransactions(loadTransactions(activeUser), activeMonthKey);
    const balances = applyMonthTransactions(startingBalances, monthTransactions).balances;
    const goals = resolveGoals(formData || {});
    ...
    if (monthStatus === "ready_to_start") {
      list.innerHTML =
        '<li class="action-item action-item--empty">Démarre le nouveau mois pour générer les actions SmartSave.</li>';
      ...
      setText("[data-actions-progress-note]", "Le nouveau mois est prêt à démarrer.");
      return;
    }
