(function (root, factory) {
  if (typeof module === "object" && module.exports) {
    ...
    root.SmartSaveTaxEngine = factory();
  }
  ...
  function calculateAnnualTax(formState, context = {}) {
    ...
    const paysTaxes = taxes.paysTaxes !== "non";
    ...
    const status = determineStatus(personal, incomes);
    ...
    if (paysTaxes) {
      fribourgTax = computeCantonalFribourgTax(taxableAfterChildDeduction, status);
      const federalTaxable = Math.max(
        0,
        taxableAfterChildDeduction - childrenCount * FEDERAL_CHILD_DEDUCTION
      );
      federalTax = computeFederalTax(federalTaxable, status);
      if (childrenCount > 0) {
  ...
  function computeAnnualNetIncome(entries) {
    ...
    return entries.reduce((sum, entry) => {
      ...
      let annual = amount * months;
      if (String(entry?.amountType).toLowerCase() === "brut") {
        annual *= 1 - SOCIAL_CHARGE_RATE;
      }
      return sum + annual;
    }, 0);
  }
  ...
  function computeAllDeductions({
    ...
    const debtDeduction = computeDebtDeduction(credits.loans || []);
    if (debtDeduction) {
      total += debtDeduction;
      breakdown.debts = debtDeduction;
    }
  ...
  function computeInsuranceDeduction(taxes, childrenCount = 0, hasSpouse = false) {
    ...
    const adultSpouse = hasSpouse
      ? toNumber(taxes.basicInsuranceSpouse) || INSURANCE_DEFAULT_PER_ADULT
      : 0;
  ...
  function determineStatus(personal, incomes) {
    const marital = (personal?.maritalStatus || "").toLowerCase();
    if (
      marital === "marie" ||
      marital === "marié" ||
      marital === "partenariat" ||
      marital === "partenariat enregistré" ||
      toNumber(incomes?.spouseNetIncome) > 0 ||
  ...
  function computeCantonalFribourgTax(income, status) {
    if (!income) return 0;
    const segments = status === STATUS_MARRIED
